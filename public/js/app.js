...
$(function ($) {
  ...
  var alertSuccess = $('#alert-success'); // send certificate code for validation & activation

  $('#cert-activate').on('submit', function (e) {
    e.preventDefault();
    alertDanger.addClass('hidden').find('ul').empty();
    alertSuccess.addClass('hidden').find('p').text('');
    var ul = alertDanger.addClass('hidden').find('ul');
    $.ajax({
      ...
      if (res.success) {
        alertSuccess.removeClass('hidden').text('Your certificate was activated!');
      } else {
  ...
  var total = $('#total');
  var amount = $('#amount');
  var plantation = $('#plantation-id');
  amount.on('change', function () {
    var amountVal = +amount.val();

    if (amountVal > 0) {
      var _treePrice = +plantation.find('option:selected').data('price');

      total.html('&euro;' + amountVal * _treePrice);
    }
  }); // change trees amount
  ...
  $('#btn-group-amount').on('click', 'button', function (e) {
    ...
    var treePrice = +plantation.find('option:selected').data('price');
  ...
  $('#cert-create').on('submit', function (e) {
    ...
    var treePrice = +plantation.find('option:selected').data('price');
    var amountVal = +amount.val();
    var totalVal = amountVal * treePrice;
    ...
    $.ajax({
      ...
      if (res.success) {
        alertSuccess.removeClass('hidden').text('You bought this certificate!');
      } else {
        if (res.hasOwnProperty("responseJSON")) {
          showErrors(res.responseJSON);
        }
...
function showErrors(responseJSON) {
  var alertDanger = $('#alert-danger');
  var ul = alertDanger.find('ul');

  if (responseJSON.hasOwnProperty("msg")) {
    ul.append("<li>".concat(responseJSON.msg, "</li>"));
    ...
    for (var key in errors) {
      if (errors.hasOwnProperty.call(errors, key)) {
        ...
        element.forEach(function (messageError) {
          ul.append("<li>".concat(messageError, "</li>"));
        });
      }
    }
  }
